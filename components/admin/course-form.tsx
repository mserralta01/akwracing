...
export default function CourseForm({ initialData }: CourseFormProps) {
  const [equipment, setEquipment] = useState<(Equipment & { id: string })[]>([]);
  ...
  useEffect(() => {
    if (initialData?.imageUrl) {
      setPreloadedImage({
        ...
        name: initialData.imageUrl.split('/').pop() || 'current-image',
        size: 0,
        type: 'image/*',
      });
    }
  ...
  useEffect(() => {
    const loadEquipment = async () => {
      ...
      try {
        ...
        // Filter out equipment without IDs
        setEquipment(data.filter((item): item is Equipment & { id: string } => !!item.id));
      } catch (error) {
